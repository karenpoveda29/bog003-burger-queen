...
import "../../styles/takingOrders.css";
...
export const TakingOrders = ({ title, menuProducts }) => {
  ...
  const prices = summaryProducts.map(({ id, quantity, addOns }) => {
    ...
    const addonsPrice = addOns.reduce(
      (acumulador, addOn) => acumulador + addOn.addOnPrice,
      0
    );
  ...
  const handleAddProductToSummary = (id) => {
    ...
    setSummaryProducts([
      ...summaryProducts,
      { id, quantity: 1, protein: selectedOption, addOns: selectedAddons },
    ]);
